...
## ▶ RESUME HERE (checkpoint 2026-07-10)

**Status: Phases 0–8 all ✅, plus the final audit + remediation pass.** The phased build
described in section B is finished. Every phase branch has been merged and deleted;
`dev` and `master` are level, and all phase artefacts (`compliance/manifest.py`,
`readiness.py`, `drift.py`, `pii.py`, `regwatch.py`, `copilot.py`, `evidence.py`,
`connectors/`, `mcp_server/`, `saas/app/rbac.py`) live on master's lineage.

**Tests:** full suite **310 passed, 0 failed** (`3.11_venv/bin/python -m pytest -q`).
Takes ~9min because of weasyprint — for a fast loop use `-o addopts=""` plus target files.

**Current branch:** `fix/site-issues` (landing-page fixes, off master). No phase work here.

**NEXT: not a phase — the build is done.** The remaining blocker is *professional legal
review*, not engineering. `LEGAL_REVIEW_NEEDED.md` has **44 items, 0 signed off**. Both
rulepack families ship scored and role-gated, but DPDP's `SOURCES_ACT.md` is still marked
DRAFT and the EU packs are secondary-sourced. Nothing further should be claimed as
"compliant" until counsel signs those off. Work that is genuinely still open and *is*
engineering, deferred during Phase 8:

- EU posture predicates and the OSS rule-mapping (built, pending legal confirmation).
- Prohibited-system hard stop — an EU-prohibited system still merely scores rules rather
  than returning a blocking verdict. See [[complisense-engine-gaps]].
- Deeper semantic artefact checks; `compliance/cross_document.py` consistency checks are
  minimal and opt-in, not comprehensive.
- regwatch LLM pipeline, MCP user-auth, offline copilot path.

To resume: read section C bottom-up + memories [[complisense-resume-status]],
[[complisense-engine-gaps]], [[complisense-positioning]]. Discipline is unchanged — propose
one feature, wait for go, build, report, STOP.

---

## A. END-TO-END AUDIT FINDINGS (2026-06-26, read-only, nothing changed)

> **Historical snapshot — do not read the unticked boxes as open work.** These findings
> were remediated across Phases 0–8; see `AUDIT_REPORT.md` and the section C log. Spot-checked
> 2026-07-10: H1 `verdict_from_score()` now returns readiness language, H2/H3 all 4 rulepacks
> carry `applicability` + `act_citation`, H4 `LEGAL_REVIEW_NEEDED.md` exists, M1 `docs/SOURCES.md`
> + `SOURCES_ACT.md` exist. The boxes are left unticked as the original audit's record.
...
- 2026-07-10 — **Landing-page fixes (`fix/site-issues`, commit 432a574).** Not phase work.
  ParticleSphere: three.js throws synchronously out of the `WebGLRenderer` constructor when
  no GL context exists (headless Chrome, blocklisted GPUs, per-page context caps); with no
  error boundary above it that throw unmounted the whole page. Added a pre-mount probe + a
  `CanvasErrorBoundary`. Readiness tool: the fixed questionnaire cost a round-trip to a
  free-tier Render instance that cold-starts ~30s, hitting exactly the first-time visitor
  the tool converts — it now ships bundled (`questionnaire.json`, generated by
  `scripts/export_questionnaire.py`, drift-pinned by
  `test_manifest.py::test_static_questionnaire_json_matches_source`), scoring still calls
  the API, `/api/health` pinged on mount, slow-call notice after 3s. Readiness form styled
  (native controls kept, painting overridden). ProductShowcase given tablist/tab/tabpanel
  roles, arrow+Home/End nav, roving tabindex, `prefers-reduced-motion`, hover/focus suspend,
  6s dwell. Stopped tracking `tsconfig.tsbuildinfo`. Full suite **310 passed**; `tsc` and
  `next build` clean.
