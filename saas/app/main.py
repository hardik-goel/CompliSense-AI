...
def _is_html_app_path(path: str) -> bool:
    return (
        ...
        or (path.startswith("/projects/") and path.endswith("/artefacts"))
    )
...
@app.get("/projects/{project_id}/artefacts", response_class=HTMLResponse)
async def project_artefacts_page(project_id: str, request: Request):
    user = _get_user_from_request(request)
    if not user:
        return RedirectResponse(url="/")
    project = projects_collection().find_one({"id": project_id, "user_id": user["id"]})
